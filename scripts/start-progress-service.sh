...
# ============================================================================
# LOCAL DEVELOPMENT SCRIPT - Progress Service Setup (No Docker)
# ============================================================================
...
PROGRESS_SERVICE_DIR="$PROJECT_ROOT/backend/progress-service"
RUNTIME_DIR="$PROJECT_ROOT/.local-dev"

mkdir -p "$RUNTIME_DIR"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  LOCAL DEVELOPMENT - Progress Service (No Docker)${NC}"
echo -e "${BLUE}========================================${NC}\n"

# Step 0: Load environment
if [ -f "$PROGRESS_SERVICE_DIR/.env" ]; then
	echo -e "${YELLOW}Using existing .env at backend/progress-service/.env${NC}"
	export $(grep -v '^#' "$PROGRESS_SERVICE_DIR/.env" | xargs -d '\n' -0 2>/dev/null || true)
else
	echo -e "${YELLOW}.env missing. Creating from env.example...${NC}"
	cp "$PROGRESS_SERVICE_DIR/env.example" "$PROGRESS_SERVICE_DIR/.env"
	echo -e "${YELLOW}Please review backend/progress-service/.env for correctness.${NC}"
	export $(grep -v '^#' "$PROGRESS_SERVICE_DIR/.env" | xargs -d '\n' -0 2>/dev/null || true)
fi

# Provide defaults if not set
DATABASE_URL=${DATABASE_URL:-"postgresql://progress_user:progress_password@localhost:5432/progress_db?schema=public"}
...
# Step 3: Ensuring Postgres database exists...
...
echo -e "  - Postgres: ${DB_HOST:-localhost}:${DB_PORT:-5432} (db: ${DB_NAME:-progress_db})"
...
echo -e "${YELLOW}Step 7: Starting Progress Service on http://localhost:${PORT}${NC}"
npm run dev
